...
export async function getDoctorAppointments() {
  ...
  try {
    ...
    if (!doctor) {
      ...
      throw new Error("Doctor account not found. Please complete your profile setup.");
    }

    // Check verification status
    if (doctor.verificationStatus !== "VERIFIED") {
      console.error(`Doctor ${doctor.id} (${doctor.name}) is not verified. Status: ${doctor.verificationStatus}`);
    ...
    if (error.message.includes("Unauthorized")) {
      ...
      throw new Error("Your account is pending verification. Please wait for admin approval.");
    } else if (error.message.includes("profile setup")) {
      throw new Error("Please complete your profile setup to view appointments");
    } else {
      throw new Error("Unable to fetch appointments. Please try again later.");
    }
  }
}
